...
import 'entrada_screen.dart'; // To reuse _TransactionDetailSheet or define its own
...
class _AllEntriesScreenState extends State<AllEntriesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final paginatedEntries = filteredEntries.isEmpty ? [] : filteredEntries.sublist(startIndex, endIndex);
        ...
        children: [
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: AppColors.azulPrincipal.withValues(alpha: isDark ? 0.15 : 0.05), shape: BoxShape.circle),
                      ...
                      children: [
                        ...
                        if (filteredEntries.isEmpty) ...[
                          const Padding(
                            ...
                            child: Center(child: Text('No hay entradas registradas.', style: TextStyle(color: Colors.white54))),
                          )
                        ] else ...[
                          // Rango de items
                          Row(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Todas las Entradas', style: TextStyle(color: AppColors.getTextColor(context), fontSize: 22, fontWeight: FontWeight.bold)),
                Text('Historial completo de ingresos', style: TextStyle(color: AppColors.getSubtextColor(context), fontSize: 13)),
              ],
            ),
          ),
        ],
        ...
        ...List.generate(totalPages, (index) {
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              height: 38,
              decoration: BoxDecoration(
                color: isSelected ? Colors.green : AppColors.getCardColor(context),
                shape: BoxShape.circle,
                border: Border.all(color: isSelected ? Colors.green : Colors.white.withValues(alpha: 0.05)),
                boxShadow: isSelected ? [BoxShadow(color: Colors.green.withValues(alpha: 0.3), blurRadius: 8)] : null,
              ),
  ...
  void _showDetails(BuildContext context, Map<String, dynamic> tx) {
    showModalBottomSheet(
      ...
      builder: (context) {
        return _TransactionDetailSheet(tx: tx, isEntrada: true);
      },
    );
  }
}
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // ─── SECCIÓN PEPS (para SALIDAS y ENTRADAS) ───
              const SizedBox(height: 20),
              const Divider(color: Colors.white10, height: 1),
              const SizedBox(height: 15),
              Row(
