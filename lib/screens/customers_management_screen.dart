...
class _CustomersManagementScreenState extends State<CustomersManagementScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredCustomers = inventoryProvider.customers.where((c) {
      final name = (c['name'] ?? '').toString().toLowerCase();
      final taxId = (c['taxId'] ?? c['tax_id'] ?? '').toString().toLowerCase();
      return name.contains(_searchQuery.toLowerCase()) || taxId.contains(_searchQuery.toLowerCase());
    }).toList();
    ...
    return Scaffold(
          ...
          SafeArea(
            child: Column(
              children: [
                ...
                Expanded(
                  child: inventoryProvider.isLoading && inventoryProvider.customers.isEmpty
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Nuevo Cliente', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Administra tu cartera de compradores',
                  style: TextStyle(
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.people_alt_rounded, size: 50, color: AppColors.moradoPrincipal),
          ),
          ...
          Text(
            'Agrega un cliente para tus transacciones de venta.',
  ...
  Widget _buildCustomerCard(BuildContext context, Map<String, dynamic> c) {
    final String id = c['id'].toString();
    final String name = c['name'] ?? 'Cliente';
    final String taxId = c['taxId'] ?? c['tax_id'] ?? 'Sin ID/RUC';
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.people_alt_rounded, color: AppColors.moradoPrincipal, size: 22),
          ),
          const SizedBox(width: 15),
          Expanded(
            child: Column(
          ...
          Row(
            ...
            children: [
              _buildCardActionButton(
                ...
                Icons.edit_rounded,
                Colors.blueAccent,
                onTap: () => _showCustomerModal(context, customer: c),
              ),
  ...
  void _showCustomerModal(BuildContext context, {Map<String, dynamic>? customer}) {
    final isEdit = customer != null;
    final nameController = TextEditingController(text: isEdit ? customer['name'] : '');
    final taxIdController = TextEditingController(text: isEdit ? (customer['taxId'] ?? customer['tax_id'] ?? '') : '');

    showGeneralDialog(
      context: context,
      barrierDismissible: true,
      barrierLabel: 'CustomerModal',
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildModalInputField(
                      ...
                      hint: 'Identificación Fiscal (ID / RUC)',
                      ...
                      children: [
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            ...
                            if (isEdit) {
                              success = await provider.updateCustomer(customer['id'].toString(), name, taxId);
                            } else {
                              success = await provider.addCustomer(name, taxId);
                            }
                            ...
                            if (context.mounted) {
                              ...
                              SweetAlert.show(
                                ...
                                message: success
                                    ? (isEdit ? 'Cliente actualizado con éxito.' : 'Cliente creado con éxito.')
                                    : (provider.errorMessage ?? 'Error al guardar cliente.'),
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: const Text('¿Eliminar Cliente?', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
              ...
              onPressed: () async {
                ...
                if (context.mounted) {
                  ...
                  SweetAlert.show(
                    ...
                    message: success ? 'Cliente eliminado exitosamente.' : (provider.errorMessage ?? 'Error al eliminar cliente.'),
                    type: success ? SweetAlertType.success : SweetAlertType.error,
                  );
                }
              },
