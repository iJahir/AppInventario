...
## 💎 Características de la Versión 2.2 (Última Actualización - Seguridad y Registro)

En esta actualización se reforzó la seguridad de las credenciales del sistema y se flexibilizó el flujo de registro e inicio de sesión:

1. **Encriptación de Contraseñas con `bcryptjs`:**
   * Todas las contraseñas se almacenan ahora de manera segura utilizando hashes de bcrypt.
   * Se incluye un proceso de migración automática al levantar el backend: las contraseñas en texto plano preexistentes en la base de datos se detectan y encriptan inmediatamente de forma automática sin afectar las cuentas de prueba creadas anteriormente.
2. **Registro e Inicio de Sesión por Nombre de Usuario (`username`):**
   * El formulario de registro en la aplicación Flutter ahora solicita un **Nombre de usuario** único además de nombre, correo y contraseña.
   * La pantalla de inicio de sesión permite acceder utilizando indistintamente el **Correo electrónico** o el **Nombre de usuario**, facilitando la usabilidad.
3. **Flujo de Recuperación por Correo Real con `nodemailer`:**
   * Al solicitar recuperar la contraseña, además de generar y persistir la clave temporal de forma segura (hasheada con bcrypt), el backend Node.js utiliza la librería `nodemailer` para enviar un correo electrónico con un diseño profesional y la clave temporal. En desarrollo, utiliza cuentas dinámicas de Ethereal SMTP para una previsualización de correo segura.
