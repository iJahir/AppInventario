...
## 💎 Historial de Versiones y Actualizaciones Recientes

### Versión 2.2 (Seguridad y Registro)
* **Encriptación con `bcryptjs`:** Almacenamiento seguro de contraseñas por hashing y migración automática de credenciales antiguas en texto plano al iniciar el servidor.
* **Nombre de Usuario (`username`):** Registro de nombre de usuario e inicio de sesión flexible por correo o usuario.
* **Recuperación por Correo Real:** Integración con `nodemailer` y plantillas HTML para el envío de claves temporales por correo electrónico.
* **Alertas Inteligentes:** Validación en pantalla de cambio de clave utilizando modales interactivos `SweetAlert`.

### Versión 2.1 (Interactividad y SweetAlerts)
* **SweetAlerts Consistentes:** Migración de SnackBars a modales interactivos `SweetAlert` en el módulo de respaldos y transferencias.
* **Acceso General a Ajustes:** Remoción de la restricción de rol `ADMIN` para acceder a la pantalla de Ajustes.
* **Triple Pulsación para Cierre de Sesión:** Cierre de sesión seguro y SweetAlert de confirmación al presionar 3 veces la tecla "Atrás" en el Dashboard.
* **Trazabilidad en Dashboard:** Acceso interactivo al desglose PEPS desde la actividad reciente del Dashboard.

### Versión 2.0 (Pro - Rendimiento y Lotes)
* **Rendimiento Zero-Lag:** Carga asíncrona en segundo plano de datos para eliminar pantallas de carga.
* **Simulación PEPS en Salidas:** Vista previa en tiempo real de lotes a consumir y margen de ganancia.
* **Seguridad por Inactividad:** Cierre de sesión automático tras 5 minutos de inactividad.
* **QR en Dispositivos Reales:** Descargas PDF de códigos QR guardados automáticamente en la carpeta física de descargas del celular.
